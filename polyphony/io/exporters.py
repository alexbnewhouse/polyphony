...
def export_replication_package(
    ...
    docs = fetchall(conn, "SELECT id, filename, content_hash, char_count, word_count, metadata FROM document WHERE project_id = ?", (project_id,))
    _write_csv(data_dir / "documents.csv", docs)

    segs = fetchall(conn, "SELECT id, document_id, segment_index, text, segment_hash, is_calibration FROM segment WHERE project_id = ? ORDER BY document_id, segment_index", (project_id,))
