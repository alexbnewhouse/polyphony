...
def export_replication_package(
    ...
    audio_source_paths = []
    for doc in docs:
        meta = from_json(doc.get("metadata"), {})
        if not isinstance(meta, dict):
            continue
        source_audio_path = meta.get("source_audio_path")
        if source_audio_path:
            audio_source_paths.append(source_audio_path)

    if audio_source_paths:
        audio_dir = output_dir / "audio"
        audio_dir.mkdir(exist_ok=True)
        copied_audio = 0
        seen_audio = set()
        for source_audio_path in audio_source_paths:
            src = Path(source_audio_path)
            if not src.exists() or src in seen_audio:
                continue
            seen_audio.add(src)
            ...
            if not dst.exists():
                ...
                copied_audio += 1
        console.print(f"  audio/: {copied_audio} source file(s) copied")
    ...
    audio_transcript_count = 0
    for doc in docs:
        meta = from_json(doc.get("metadata"), {})
        if isinstance(meta, dict) and meta.get("source_type") == "audio_transcription":
    ...
    manifest = {
        ...
        "corpus_stats": {
            ...
            "audio_transcript_count": audio_transcript_count,
...
| `audio/` | Source audio files for transcribed documents (if any) |
